...
export function toranporin(ctx: CanvasRenderingContext2D) {
  const text = "猫のトランポリン";
  const textSize = 48;
...
export function drawTitle(ctx: CanvasRenderingContext2D, time: timestamp) {
  ...
  console.log(Math.sin(time) * Math.PI);
  const textScale = 5;
  const pos = new Point2(200, 200);
